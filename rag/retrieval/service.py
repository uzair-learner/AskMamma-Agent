...
from rag.embeddings.provider import embedding_backend_name
from rag.vectorstore.faiss_store import load_store, save_documents, vector_store_exists
...
def sanitize_filename(file_name: str) -> str:
    ...
    if Path(safe).suffix.lower() not in SUPPORTED_EXTENSIONS:
        raise ValueError("Unsupported file type. Use PDF, TXT, DOCX, Markdown, or CSV.")
    return safe
...
def rebuild_vector_store() -> dict[str, Any]:
    return save_documents(_build_documents_from_database())
...
def reindex_documents() -> dict[str, Any]:
    ...
    indexed = [ingest_document(path, rebuild_index=False) for path in _iter_document_paths()]
    return {"indexed": indexed, "count": len(indexed), "vector_store": rebuild_vector_store()}
...
def document_search(query: str, limit: int = 5) -> dict[str, Any]:
    ...
    matches = load_store().similarity_search_with_score(query, k=limit)
    results = [
        {
    ...
    return {"found": True, "results": results, "retriever": f"faiss+{embedding_backend_name()}"}
