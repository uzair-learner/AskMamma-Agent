"""FastAPI backend for the Inventory Pilot AI agent system."""
...
from inventory_pilot_ai.api.auth import router as auth_router
from inventory_pilot_ai.api.dependencies import get_current_user, optional_current_user, require_permission
...
from inventory_pilot_ai.llm_provider import (
    LLM_UNAVAILABLE_MESSAGE,
    current_runtime_status,
    get_llm_provider,
    validate_ollama_configuration,
)
from inventory_pilot_ai.observability import configure_langsmith, safe_error_message
from inventory_pilot_ai.db.database import (
    create_product,
...
from inventory_pilot_ai.memory.service import audit_memory, conversation_memory, semantic_memory
from inventory_pilot_ai.protocols.a2a.service import create_task_record, utcnow
from inventory_pilot_ai.protocols.agent_cards.builder import build_agent_card
from inventory_pilot_ai.protocols.mcp.server import handle_rpc, list_prompts, list_resources, list_tools
...
def _orchestrator():
    from inventory_pilot_ai.agents import orchestrator

    return orchestrator


def _rag():
    from inventory_pilot_ai.rag import retriever
...
def reports_demo(user: dict[str, Any] = Depends(require_permission("report:write"))) -> dict[str, Any]:
    return reports_inventory_pilot_ai(user)


@app.get("/reports/inventory-pilot-ai")
def reports_inventory_pilot_ai(user: dict[str, Any] = Depends(require_permission("report:write"))) -> dict[str, Any]:
    tenant_id = int(user["tenant_id"])
