...
DATABASE_URL = env("DATABASE_URL", f"sqlite:///{(ROOT_DIR / 'inventory_pilot_ai.db').as_posix()}")
LLM_PROVIDER = env("LLM_PROVIDER").lower()
...
LANGSMITH_PROJECT = env("LANGSMITH_PROJECT", "Inventory-Pilot-AI")
...
AGENT_TIMEOUT_SECONDS = int(env("AGENT_TIMEOUT_SECONDS", "12"))
JWT_SECRET = env("JWT_SECRET", "dev-only-change-me")
JWT_ISSUER = env("JWT_ISSUER", "inventory-pilot-ai")
JWT_AUDIENCE = env("JWT_AUDIENCE", "inventory-pilot-ai-users")
