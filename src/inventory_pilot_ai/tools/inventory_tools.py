"""Typed Inventory Pilot AI demo tools exposed to LangChain, FastAPI, and MCP adapters."""
...
class DocumentSearchInput(BaseModel):
    query: str = Field(..., min_length=1, description="Question or search string for Inventory Pilot AI documents.")
    limit: int = Field(default=5, ge=1, le=10, description="Maximum number of retrieved chunks.")


class DemoReportInput(BaseModel):
    title: str = Field(default="Inventory Pilot AI Operations Report", min_length=3, max_length=120)
...
def write_demo_report(title: str = "Inventory Pilot AI Operations Report", output_format: str = "md") -> dict[str, Any]:
    ...
    file_name = f"Inventory Pilot AI-report-{timestamp}.{output_format}"
    ...
    if output_format == "json":
        ...
        if output_format != "xlsx":
            output_format = "xlsx"
            path = config.REPORT_DIR / f"Inventory Pilot AI-report-{timestamp}.xlsx"
...
def tool_registry() -> list[ToolCall]:
    return [
        ToolCall(
            ...
            description="Search sample Inventory Pilot AI demo items by name, SKU, category, partner, or description.",
        ...
        ToolCall(
            ...
            description="Search indexed Inventory Pilot AI documents using local embedding retrieval.",
            input_schema=DocumentSearchInput.model_json_schema(),
            output_schema={"type": "object"},
        ),
        ToolCall(
            name="DemoReportWriterTool",
            description="Generate a sample Inventory Pilot AI operations Excel report and save it under outputs/reports.",
        ...
        ToolCall(
            name="AuditLogTool",
            description="Persist a sanitized local audit record for the current Inventory Pilot AI session.",
